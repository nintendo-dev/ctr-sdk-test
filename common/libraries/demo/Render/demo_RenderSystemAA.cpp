...
#include "demo/Render/demo_RenderSystemAA.h"
...
void
RenderSystemAA::Initialize(const uptr						fcramAddress,
						   ...
						   const bool						isFillBlackLCD)
{
	...
	if (m_AAMode == AA_2x1)
	{
		for (size_t i = 0; i < DEMO_RENDER_BUFFER_NUM; i++)
		{
...
void
RenderSystemAA::SetRenderTarget(const GLenum targetDisplay)
{
...
void
RenderSystemAA::SetUnitViewport(f32 Left, f32 Right, f32 Bottom, f32 Top)
{
	m_Left	 = Left;
	m_Right	 = Right;
	m_Bottom = Bottom;
