...
extern u64 demoFontBitmap[];

/* ------------------------------------------------------------------------
		<tt>FontProgram</tt> Class Member Function
   ------------------------------------------------------------------------ */

FontProgram::FontProgram(void) : ProgramObject(), m_FontTextureId(0)
{
	InitializeVertexAttributes();
}

FontProgram::~FontProgram(void)
{
	Finalize();
}

bool
FontProgram::Initialize(const GLuint shaderId)
{
	if (m_Initialized)
	{
		return false;
...
FontProgram::InitializeFontTexture(void)
{
	...
	for (u32 i = 0; i < 128; i++)
	{
		for (u32 j = 0; j < 8; j++)
		{
			u64 pixels = (*(demoFontBitmap + offset));
