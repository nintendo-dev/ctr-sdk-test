...
namespace detail {
...
#if DEBUG_PRINT
		NN_TLOG_("[demo::MemoryManager] Start address on Main Memory (FCRAM): 0x%08X\n", m_pStartAddrFcram);
		NN_TLOG_("[demo::MemoryManager] Start address on VRAM-A: 0x%08X\n", m_CurrentAddrVramA);
		NN_TLOG_("[demo::MemoryManager] Start address on VRAM-B: 0x%08X\n", m_CurrentAddrVramB);
#endif
	...
	void MemoryManager::PrintFreeMemorySize(void)
	{
		...
		NN_TLOG_("[demo::MemoryManager] Free memory size on VRAM-A is 0x%06X(max 0x%06X)\n",
				 nn::gx::GetVramEndAddr(nn::gx::MEM_VRAMA) - m_CurrentAddrVramA,
				 nn::gx::GetVramSize(nn::gx::MEM_VRAMA));
		NN_TLOG_("[demo::MemoryManager] Free memory size on VRAM-B is 0x%06X(max 0x%06X)\n",
	...
	void* MemoryManager::Allocate(GLenum area, GLenum aim, GLuint id, GLsizei size)
	{
		...
		{
			NN_TLOG_("[demo::MemoryManager] NN_GX_MEM_SYSTEM is allocated at 0x%08X - 0x%08X on %X\n",
					 resultAddr,
		...
		{
			NN_TLOG_("[demo::MemoryManager] NN_GX_MEM_TEXTURE is allocated at 0x%08X - 0x%08X on %X\n",
					 resultAddr,
					 reinterpret_cast<int>(resultAddr) + size - 1,
					 area);
		}
		...
		{
			NN_TLOG_("[demo::MemoryManager] NN_GX_MEM_VERTEXBUFFER is allocated at 0x%08X - 0x%08X on %X\n",
					 resultAddr,
					 reinterpret_cast<int>(resultAddr) + size - 1,
					 area);
		}
		...
		{
			NN_TLOG_("[demo::MemoryManager] NN_GX_MEM_DISPLAYBUFFER is allocated at 0x%08X - 0x%08X on %X\n",
					 resultAddr,
					 reinterpret_cast<int>(resultAddr) + size - 1,
					 area);
		}
		...
		{
			NN_TLOG_("[demo::MemoryManager] NN_GX_MEM_RENDERBUFFER is allocated at 0x%08X - 0x%08X on %X\n",
					 resultAddr,
					 reinterpret_cast<int>(resultAddr) + size - 1,
					 area);
		}
		...
		{
			NN_TLOG_("[demo::MemoryManager] NN_GX_MEM_COMMANDBUFFER is allocated at 0x%08X - 0x%08X on %X\n",
					 resultAddr,
					 reinterpret_cast<int>(resultAddr) + size - 1,
					 area);
		}
...
#if DEBUG_PRINT
		NN_TLOG_("[demo::MemoryManager] (id %d) was deallocated at 0x%08X on %X\n", id, addr, area);
#endif
...
namespace {
	demo::detail::MemoryManager s_MemoryManager;
}

NN_WEAK_SYMBOL void
InitializeMemoryManager(const uptr fcramAddress, const size_t memorySize)
{
	s_MemoryManager.Initialize(fcramAddress, memorySize);
}

NN_WEAK_SYMBOL void
FinalizeMemoryManager(void)
{
	s_MemoryManager.Finalize();
}

NN_WEAK_SYMBOL void
PrintMemoryManagerInfo(void)
{
	s_MemoryManager.PrintFreeMemorySize();
}

NN_WEAK_SYMBOL void*
GetAllocator(GLenum area, GLenum aim, GLuint id, GLsizei size)
{
	return s_MemoryManager.Allocate(area, aim, id, size);
}

NN_WEAK_SYMBOL void
GetDeallocator(GLenum area, GLenum aim, GLuint id, void* addr)
{
	s_MemoryManager.Deallocate(area, aim, id, addr);
}

NN_WEAK_SYMBOL void*
Alloc(size_t size)
{
	return s_MemoryManager.Allocate(NN_GX_MEM_FCRAM, NN_GX_MEM_SYSTEM, 0, size);
}

NN_WEAK_SYMBOL void
Free(void* ptr)
{
	s_MemoryManager.Deallocate(NN_GX_MEM_FCRAM, NN_GX_MEM_SYSTEM, 0, ptr);
}
