...
bool
ConvertGLTextureToNative(const GLenum format, const u32 width, const u32 height, void* glData, void* dmpData)
{
	bool		yflip = true;
 ...
 */
static s32
GetPixelByte(const GLenum format)
{
...
static s32
WriteBlock(u32 no, const u8* pSrc, TextureInformation* textureInformation)
{
	u32 row		  = (no / textureInformation->blocksInRow);
	u32 column	  = (no % textureInformation->blocksInRow);
	u32 pixelByte = GetPixelByte(textureInformation->format);
...
static s32
DeBlock(const u8* pSrc, u8* pDst, s32 blocksize, TextureInformation* textureInformation)
{
	u32 pixelByte = GetPixelByte(textureInformation->format);
...
static s32
EnBlock(const u8* pSrc, u8* pDst, s32 blocksize, TextureInformation* textureInformation)
{
	u32 pixelByte = GetPixelByte(textureInformation->format);
	...
	if (blocksize != 1)
	{
		...
		if (!textureInformation->yflip)
		{
			...
			EnBlock(pSrc + (pixelByte * blocksize * TEXTURE_BLOCK_SIZE), pDst + (pixelByte * pixels * 2), blocksize, textureInformation);
		...
		else
		{
			...
			EnBlock(pSrc + (pixelByte * blocksize * TEXTURE_BLOCK_SIZE), pDst + (pixelByte * pixels * 0), blocksize, textureInformation);
...
static s32
SwapColorComponents(TextureInformation* textureInformation)
{
...
static s32
ConvertFormat8888Function(TextureInformation* textureInformation)
{
	u8	tmp;
	u32 pixelByte = GetPixelByte(textureInformation->format);
...
static s32
ConvertFormat888Function(TextureInformation* textureInformation)
{
	u8	tmp;
	u32 pixelByte = GetPixelByte(textureInformation->format);
	s32 total	  = static_cast<s32>(textureInformation->width * textureInformation->height * pixelByte);
...
static s32
ConvertFormat88Function(TextureInformation* textureInformation)
{
	u8	tmp;
	u32 pixelByte = GetPixelByte(textureInformation->format);
	s32 total	  = static_cast<s32>(textureInformation->width * textureInformation->height * pixelByte);
...
static s32
ConvertFormatXXXXFunction(TextureInformation* textureInformation)
{
